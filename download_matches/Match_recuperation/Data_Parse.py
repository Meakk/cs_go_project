...
def match_recuperation_dict_txt(
    ...
    for i in range(len(all_match_player["items"])):
        ...
        for root, dirs, files in os.walk(
            "demo_csgo/DataBase/" + match_details["voting"]["map"]["pick"][0]
        ):
        ...
        proc.append(
            multiprocessing.Process(
                target=download_parse,
                args=(url, match_details, all_match_player, nickname, i),
            )
        )
        proc[succeed].start()
        succeed += 1
...
def download_parse(url, match_details, all_match_player, nickname, i):
    ...
    try:
        ...
        demo_parser = DemoParser(
            demofile="demo_csgo/DataBase/" + match_name + ".dem",
            demo_id=nickname
            + "_"
            + match_details["voting"]["map"]["pick"][0]
            + "_"
