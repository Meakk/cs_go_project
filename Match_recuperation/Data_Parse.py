...
def match_recuperation_dict_txt(api_key="38b28095-4ca6-48b6-aec5-748f507d5fcf",
    ...
    for i in range(len(all_match_player["items"])):
       try:
            verif = 0
            print("_______________START DOWNLOADING MATCH NUMBER :", i, "___SUCCEED BEFORE : ", succeed)
            ...
            for root, dirs, files in os.walk("C:/demo_csgo/DataBase/" + match_details["voting"]["map"]["pick"][0]):
                for filename in files:
                    # print(filename," compare to : ",match_details["voting"]["map"]["pick"][0]+"_"+str(match_details["configured_at"])+"_"+match_name+'.txt')
                    if filename == match_details["voting"]["map"]["pick"][0] + "_" + str(
            ...
            with open('C:/demo_csgo/DataBase/' + match_details["voting"]["map"]["pick"][0] + '/' +
